...
(function () {
					...
					if (requestTab == null || (requestTab != null && windowId != null) || window.tabs[j].id != requestTab.id)
						if (isTabURLAllowedForPark(window.tabs[j]))
							if (!isExceptionTab(window.tabs[j]))
								parkTab(window.tabs[j], window.tabs[j].id, {
									bulkNumber: (window.tabs[j].discarded ? number++ : null)
								});
		};
				...
				try {
					...
					if (tab != null /*&& !tab.active && isTabURLAllowedForPark(tab)*/ ) {
						t5.sendMessage(tabId, {
							method: '[AutomaticTabCleaner:getOriginalFaviconUrl]'
						}, function (originalFaviconUrl) {
...

							{
								...
								if (debug) {
									if (isTabURLAllowedForPark(tab) && tab.discarded == false) {
										try {
											t5.executeScript(tabId, {
												code: 'document.title = \'' + appendTitleDebug(tab.title, tabInfo) + '\''
											});
											// eslint-disable-next-line no-empty
										} catch (e) {}
									}
	...
	r5.onMessage.addListener(function (request, sender, sendResponse) {
		...
		/* Ð¡ase when screen moved from inject side */
			...
			sendResponse({
				isException: whiteList.isURIException(request.uri)
			});
	...
	function hideWhiteListDialod(tabId, options) {
		t5.sendMessage(tabId, {
			method: '[AutomaticTabCleaner:hideDialogRequetToTab]',
			options: options
		});
	}
			...
			try {
				if ( /*isTabURLAllowedForPark(tab) &&*/ !isTabParked(tab) && tab.url.indexOf(sessionsPageUrl) == -1) {
					if (!tab.discarded)
						(function (closureTab) {
							setTimeout(function () {
								captureTab(closureTab, {
									checkActiveTabNotChanged: true
								});
							}, 400);
						})(tab);
			...
			try {
				if (tab.url.indexOf(sessionsPageUrl) == 0)
					chrome.tabs.sendMessage(tab.id, {
						'method': '[AutomaticTabCleaner:updateSessions]'
					});
				if (tab.url.indexOf(historyPageUrl) == 0)
					chrome.tabs.sendMessage(tab.id, {
						'method': '[AutomaticTabCleaner:updateHistoryPage]'
					});
			} catch (e) {
				console.error(e);
			}
	...
	e5.onRequest.addListener(function (request, sender, sendResponse) {
		if (request.method === '[AutomaticTabCleaner:ReloadSettings]') {
			...
			sendResponse({
				settings: JSON.stringify(settings.toObject(), null, 2)
			});
		} else if (request.method === '[AutomaticTabCleaner:importAllSettings]') {
			settings.removeAll();
			settings = new Store(settingsStorageNamespace, {
	...
	function start() {
		...
		if (dbMovedFromWebSqlToIndexedDB != true) {
			webSqlDatabase = new DBProvider('WebSQl', {
